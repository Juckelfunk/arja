...
public class ClassFinder {
	...
	public ClassFinder(String binJavaDir, String binTestDir, Set<String> dependences)
		...
		binExecuteTestClasses = new LinkedHashSet<String>();
		binAbstractTestClasses = new LinkedHashSet<String>();
		binJavaClasses = new LinkedHashSet<String>();
	...
	void scanTestDir(URLClassLoader classLoader) throws ClassNotFoundException {
		List<File> files = new ArrayList<>(FileUtils.listFiles(new File(binTestDir), new SuffixFileFilter(".class"),
	...
	void scanJavaDir() throws ClassNotFoundException {
		...
		List<File> files = new ArrayList<>(FileUtils.listFiles(dir, new SuffixFileFilter(".class"),
				TrueFileFilter.INSTANCE));
		Collections.sort(files);
